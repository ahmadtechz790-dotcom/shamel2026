...
async function loadSupabaseStats() {
	...
	const { data, error } = await supabaseClient.from("visitor_stats").select("student_number, found, browser, device, country, visited_at").order("visited_at", { ascending: false }).limit(1000);
